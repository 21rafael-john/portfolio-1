    ...
    <div className="min-h-screen">
      ...
      <section id="projects" className="mx-auto max-w-6xl px-4 py-12">
        <div className="flex items-end justify-between">
          ...
          <Link href="/projects" className="text-sm inline-flex items-center gap-1 hover:opacity-80">All projects <ArrowUpRight className="w-4 h-4"/></Link>
        </div>
          ...
          {featured.map((p, idx) => {
            ...
            return (
              <motion.article key={p.slug} layout className="overflow-hidden rounded-2xl border border-slate-200 bg-white hover:shadow-lg transition-shadow">
                <button type="button" onClick={() => setOpen(isOpen ? null : idx)} onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setOpen(isOpen ? null : idx); }
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    <motion.div key="details" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                      exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="px-6 pb-6">
                      <div className="mt-4 text-sm text-slate-700 leading-relaxed">{p.long ?? p.description}</div>
              ...
              {skills.map((s) => (
                <div key={s.name} className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                  ...
                  <div className="mt-2 h-2 rounded-full bg-slate-200">
                    <div className="h-2 rounded-full bg-sky-500" style={{ width: `${s.pct}%` }} />
                  </div>
                </div>
              ))}
            </CardContent>
      ...
      <section id="about" className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mt-6 grid md:grid-cols-[1.2fr_.8fr] gap-6">
          ...
          <Card className="rounded-2xl"><CardHeader><CardTitle>Highlights</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm">
            <div className="flex items-center gap-2"><ArrowUpRight className="w-4 h-4"/> Built 10+ end-to-end mini projects</div>
            <div className="flex items-center gap-2"><ArrowUpRight className="w-4 h-4"/> Comfortable with Git & CI basics</div>
            <div className="flex items-center gap-2"><ArrowUpRight className="w-4 h-4"/> Interested in fintech & mobility</div>
          </CardContent></Card>
        </div>
      </section>
